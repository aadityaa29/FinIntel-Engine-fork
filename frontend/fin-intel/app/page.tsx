...
// ─────────────────────────────────────────────
// TYPES — aligned with backend NewsItem schema
// ─────────────────────────────────────────────
...
interface NewsItem {
  id?: string | number;
  title: string;    // Real headline from scraper — never "Market Update"
  text: string;     // Full combined text (headline + snippet)
  url?: string;     // Direct source URL — always present from updated scraper
  source?: string;
  time?: string;    // display string (relative, e.g. "2h ago")
  date?: string;    // raw backend field (YYYY-MM-DD)
  sentiment?: SentimentValue; // null coerced to undefined in normalizeNewsItem
}

interface Ticker { symbol: string; price: string; change: number; }
interface TrendingStock {
  symbol: string; name: string; price: string; change: string;
  isUp: boolean; history: number[]; volume?: string; marketCap?: string;
  category: "stocks" | "crypto" | "indian" | "us";
}
interface Insight {
  label: string; value: string; color: string; bg: string;
  conf?: number; icon?: string; signal?: "buy" | "sell" | "hold";
  risk?: "low" | "medium" | "high"; trend?: "up" | "down" | "flat"; tooltip?: string;
}
...
const MOCK_NEWS: NewsItem[] = [
  { id:"1", title:"Global Markets Rally on Stellar Tech Earnings Beat",   text:"Tech sector pushes indices to record highs as major players report Q3 earnings 15% above consensus estimates, driving significant institutional inflows. Fund managers are rotating into quality growth names as confidence returns.", url:"https://bloomberg.com", source:"Bloomberg", time:"2h ago", sentiment:"bullish" },
  { id:"2", title:"Federal Reserve Signals Potential Rate Cuts in H2 2024", text:"In a pivotal policy shift, the Fed indicated that cooling inflation data may pave the way for rate reductions later this year, sparking a bond market rally and a sharp move higher in equities.", url:"https://reuters.com",   source:"Reuters",   time:"4h ago", sentiment:"bullish" },
  { id:"3", title:"AI Semiconductor Demand Hits All-Time High",             text:"Enterprise AI adoption is accelerating globally, with chipmakers reporting record backlogs that could sustain elevated margins well into 2025. Supply constraints remain a key risk to watch.",              url:"https://wsj.com",     source:"WSJ",       time:"5h ago", sentiment:"bullish" },
  { id:"4", title:"Oil Prices Dip on China Demand Uncertainty",             text:"Brent crude slipped below $80 per barrel as weaker-than-expected Chinese PMI data raised concerns about future energy demand from the world's largest importer. Energy stocks declined broadly.", url:"https://ft.com",      source:"FT",        time:"6h ago", sentiment:"bearish" },
  { id:"5", title:"Small-Cap Rotation Gains Steam Ahead of FOMC",           text:"Institutional money is quietly rotating out of mega-cap tech into Russell 2000 constituents, a pattern historically observed before dovish Fed pivots. Volume data confirms accumulation.",    url:"https://marketwatch.com", source:"MarketWatch", time:"8h ago", sentiment:"neutral" },
];
...
/** Format a YYYY-MM-DD date from the backend into a human-readable relative string */
...
/** Normalise a NewsItem coming from the backend */
function normalizeNewsItem(item: NewsItem, index: number): NewsItem {
  return {
    ...item,
    id:   item.id ?? index,
    time: formatNewsDate(item.date || item.time),
    // Backend guarantees a real URL; keep it, never fall back to "#"
    url:  item.url || "",
    sentiment: normalizeSentiment(item.sentiment),
  };
}

function normalizeSentiment(s?: string | null): SentimentValue | undefined {
    ...
    <span>
      {parts.map((p, i) =>
        p.toLowerCase() === highlight.toLowerCase()
          ? <span key={i} className="text-blue-400 font-bold">{p}</span>
          : <span key={i}>{p}</span>
      )}
    </span>
  );
...
const SentimentPill = ({ sentiment }: { sentiment?: SentimentValue }) => {
  ...
  return (
    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border uppercase tracking-wider ${map[sentiment] || map.neutral}`}>
      {sentiment}
    </span>
  );
};
...
// ─────────────────────────────────────────────
const TickerBar = ({
  data, isLoading, onSymbolClick,
}: { data: Ticker[]; isLoading: boolean; onSymbolClick: (s: string) => void }) => {
    ...
    >
      ...
      {isLoading && !data.length ? (
        <div className="flex px-6 gap-6">
          {[...Array(7)].map((_, i) => <Skeleton key={i} className="w-28 h-4" />)}
        </div>
      ) : (
      ...
      <AnimatePresence>
        {showDropdown && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            onClick={() => { setIsFocused(false); setSymbol(""); }} />
        )}
      </AnimatePresence>
...
const TrendingAssets = ({
  data, isLoading, onSymbolClick, watchlist, onToggleWatchlist,
}: {
  data: any[]; // Replace 'any' with your 'TrendingStock' interface
  isLoading: boolean; onSymbolClick: (s: string) => void;
    ...
    <section className="mb-16">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {["All", "Tech", "Crypto", "Finance"].map(cat => ( // Replace array with CATEGORY_FILTERS
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {isLoading && !data.length
          ? [...Array(4)].map((_, i) => <div key={i} className="h-40 w-full rounded-2xl bg-white/[0.03] animate-pulse" />) // Replace with your Skeleton
          : filtered.map((stock, idx) => (
            ...
            >
              ...
              <button onClick={(e) => onToggleWatchlist(stock.symbol, e)} className="absolute top-4 right-4 z-10 transition-all active:scale-90">
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                {/* Ensure your Sparkline component is imported */}
                ...
                {expanded === stock.symbol && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                    ...
                    <div className="mt-4 pt-4 border-t border-white/[0.06] text-xs">
                      {/* Conditional rendering for shallow vs deep data */}
                      {(stock.volume || stock.fundamentals?.volume || stock.market_cap || stock.fundamentals?.market_cap || stock.marketCap) ? (
                        <div className="grid grid-cols-2 gap-2 mb-2">
                          <div className="flex flex-col gap-0.5">
                            <span className="text-gray-500">Volume</span>
                            <span className="text-white font-semibold">
                              {stock.volume || stock.fundamentals?.volume 
                                ? Intl.NumberFormat('en-US', { notation: "compact", maximumFractionDigits: 1 }).format(stock.volume || stock.fundamentals?.volume) 
                                : "—"}
                            </span>
                          </div>
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-white font-semibold">
                              {stock.market_cap || stock.fundamentals?.market_cap || stock.marketCap
                                ? Intl.NumberFormat('en-US', { notation: "compact", maximumFractionDigits: 1 }).format(stock.market_cap || stock.fundamentals?.market_cap || stock.marketCap) 
                                : "—"}
                            </span>
                          </div>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center py-3 mb-2 bg-blue-500/[0.02] border border-blue-500/[0.05] rounded-xl text-center">
                          <Activity size={16} className="text-blue-400/50 mb-1" />
                          <span className="text-[11px] text-gray-400">Deep metrics unavailable</span>
                          <span className="text-[10px] text-gray-500">Run analysis to fetch live fundamentals</span>
                        </div>
                      )}

                      <button onClick={(e) => { e.stopPropagation(); onSymbolClick(stock.symbol); }}
                        className="w-full mt-2 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-blue-500/15 text-blue-400 hover:bg-blue-500/25 font-semibold text-xs transition-colors border border-blue-500/20">
...
// ─────────────────────────────────────────────
// NEWS MODAL  — full article text + real URL
// ─────────────────────────────────────────────
const NewsModal = ({ news, onClose }: { news: NewsItem; onClose: () => void }) => (
  ...
  >
    <motion.div
      ...
      exit={{ scale: 0.95, opacity: 0 }} transition={{ type: "spring", stiffness: 400, damping: 35 }}
      className="bg-[#0e0e0e] border border-white/10 rounded-3xl p-8 max-w-2xl w-full shadow-[0_40px_100px_rgba(0,0,0,0.8)] max-h-[85vh] overflow-y-auto scrollbar-hide"
    ...
    >
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2 flex-wrap">
          {news.source && (
            <span className="text-xs font-bold bg-blue-500/20 text-blue-400 px-2.5 py-1 rounded-full border border-blue-500/20">
              {news.source}
            </span>
          )}
      ...
      {/* Title */}
      <h2 className="text-xl font-bold leading-snug mb-4 text-white">{news.title}</h2>

      {/* Full body text — no truncation */}
      <p className="text-gray-300 leading-relaxed text-sm mb-8 whitespace-pre-wrap">{news.text}</p>

      {/* CTA — opens the real source URL in a new tab */}
      {news.url ? (
        <a
          href={news.url}
          target="_blank"
        ...
        <p className="text-xs text-gray-600 italic">Source link unavailable.</p>
      )}
    </motion.div>
  </motion.div>
);
...
// ─────────────────────────────────────────────
// NEWS FEED — global market news (no ticker filter)
// ─────────────────────────────────────────────
const NewsFeed = ({
  data, isLoading, error, onRetry,
}: { data: NewsItem[]; isLoading: boolean; error?: boolean; onRetry: () => void }) => {
    ...
    <div className="md:col-span-2 bg-[#0a0a0a] border border-white/[0.06] p-6 rounded-3xl flex flex-col">
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <Globe className="text-blue-400" size={18} /> Global Market News
        </h2>
      ...
      {error ? <ErrorState onRetry={onRetry} /> : isLoading && !data.length ? (
        ...
        <div className="flex flex-col gap-3 overflow-y-auto scrollbar-hide max-h-[520px] pr-1">
          ...
          {featured && (
            ...
            >
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                {featured.source && <span className="text-[10px] font-semibold text-gray-400">{featured.source}</span>}
              ...
              {/* Full title — no truncation on featured card */}
              <h3 className="font-bold text-base leading-snug group-hover:text-blue-300 transition-colors mb-2">{featured.title}</h3>
              {/* Show up to 3 lines of body on the card */}
              <p className="text-xs text-gray-400 line-clamp-3 leading-relaxed">{featured.text}</p>
              <div className="flex items-center gap-1.5 mt-3 text-xs text-blue-400 font-medium">
                <Eye size={12} /> Read full story
              </div>
            </button>
          )}
          ...
          {rest.map((n, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                {n.source && <span className="text-[10px] font-semibold text-blue-400">{n.source}</span>}
              ...
              {/* Full headline — no clamp so titles are never cut mid-word */}
              <h3 className="font-semibold text-sm leading-snug group-hover:text-blue-300 transition-colors mb-1">{n.title}</h3>
...
const InsightsPanel = ({ data, isLoading }: { data: Insight[]; isLoading: boolean }) => {
  ...
  const trendIcon = (t?: string) =>
    t === "up"   ? <ArrowUp size={12} className="text-emerald-400" />
  : t === "down" ? <ArrowDown size={12} className="text-rose-400" />
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    <button onMouseEnter={() => setHoveredTooltip(i)} onMouseLeave={() => setHoveredTooltip(null)} className="text-gray-600 hover:text-gray-400 transition-colors">
                      <Info size={11} />
                    </button>
                    <AnimatePresence>
                      {hoveredTooltip === i && (
...
const WatchlistPanel = ({
  watchlist, onSymbolClick, onRemove,
}: { watchlist: string[]; onSymbolClick: (s: string) => void; onRemove: (s: string) => void }) => {
  ...
  <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-10 bg-white/[0.02] border border-white/[0.05] rounded-2xl px-5 py-3">
    <div className="flex items-center gap-3 flex-wrap justify-center sm:justify-start">
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className={`font-semibold ${isOpen ? "text-emerald-400" : "text-gray-400"}`}>
          NSE/BSE {isOpen ? "Open" : "Closed"}
        </span>
      </div>
...
export default function Home() {
  ...
  // Market data (tickers + trending + macro insights)
  const {
    data: marketData, isLoading: isMarketLoading,
    mutate: retryMarket,
  } = useSWR(`${API_BASE}/market`, fetcher, {
    refreshInterval: marketOpen ? 10_000 : 60_000,
    fallbackData: null,
  });

  // ── Global market news — uses /news/market endpoint (no ticker filter) ──
  const {
    data: newsData, isLoading: isNewsLoading,
    error: newsError, mutate: retryNews,
  } = useSWR(`${API_BASE}/news/market`, fetcher, {
    refreshInterval: 5 * 60_000,   // refresh every 5 min
    fallbackData: null,
  });

  // Normalise news items from backend schema → NewsItem
  const activeNewsData: NewsItem[] = (() => {
    const raw: NewsItem[] = newsData?.news ?? [];
    if (raw.length > 0) return raw.map(normalizeNewsItem);
    return MOCK_NEWS;
  })();
